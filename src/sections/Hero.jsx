  ...
  return (
    <section id='home' className='w-full flex xl:flex-row flex-col justify-center min-h-screen gap-10 max-container'>
      <div className='relative xl:w-2/5 flex flex-col justify-center items-start w-full max-xl:padding-x pt-28'>
        ...
        <div className='flex justify-start items-start flex-wrap sm:gap-16 w-full mt-20 gap-10 '>
          {statistics.map((stat) => (
            <div key={stat.label }>
      ...
      <div className='relative flex-1 flex justify-center items-center xl:min-h-screen
       max-xl:py-40 bg-primary bg-hero bg-cover bg-center'>
        <img src={bigShoeImg} width={610} height={610} alt="shoe image" 
        className='object-contain'
        />

        <div className='flex sm:gap-6 gap-4 absolute -bottom-[10%] sm:left-[10%] max-sm:px6'>
          {shoes.map((shoe) => (
            <div key={shoe}>
              <ShoesCard 
                imgURL={shoe}
                changeshoesImg={(shoe) => setBigShoeImg(shoe)}
